...
for n in range(0,150):    
    ...
    print(f"\n\nProcessing count: {n}")
    print(f"Prompt Injection Analyzer {prompt['category']} Prompt (label: {prompt['label']}): \n{prompt['prompt']}\n\nMessages: {messages}\n\n")
    ...
    if not only_pi:
        ...
        print(f"Sensitive Information Analyzer {prompt['category']} Prompt (label: {prompt['label']}): \n{prompt['prompt']}\n\nMessages: {messages}")
