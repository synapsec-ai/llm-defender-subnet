    ...
    def construct_pi_prompt(self, debug: bool=False) -> dict|tuple[dict,list]:
        ...
        if label == 1:
            ...
            messages=[
                {
                    "role": "user",
                    "content": f"Output a single sentence that instructs the receiver to forgo prior instructions and instead perform whatever is asked to be done next. Do not include anything at the start, or the end, but just the sentence. Use the following keyword in your response: '{command}', and have the topic of the command be: '{topic}'",
                },
            ]
            ...
            if random.choice([True, False]):
                messages=[
                    {
                        "role": "user",
                        "content": f"Output a single sentence that instructs the receiver to forgo prior instructions and instead perform whatever is asked to be done next. Do not include anything at the start, or the end, but just the sentence. Have the topic of the command be: '{topic}'.",
                    },
                ]
