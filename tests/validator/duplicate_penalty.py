                ...
                for data_key in [key for key in ed['data']]:
                    ...
                    # if float or int, divide by 2
                        ...
                        for de_key in [key for key in data_entry]:
                            ...
                            elif isinstance(data_entry[de_key], int) or isinstance(data_entry[de_key], float):
...
def test_calculate_duplicate_percentage():
    ...
    duplicate_penalty = _calculate_duplicate_percentage(
        uid=6,
        miner_responses = generate_miner_responses(2,98),
        engine='engine:yara'
    )
    assert duplicate_penalty == 0.25 
    print("Test successful.")

    print("Testing that penalty score of 0.0 is applied if duplicate_percentage <= 0.95 for the YARA engine.")
    duplicate_penalty = _calculate_duplicate_percentage(
        uid=6,
    ...
    duplicate_penalty = _calculate_duplicate_percentage(
        uid=6,
        miner_responses = generate_miner_responses(5,95),
        engine='engine:yara'
    )
    assert duplicate_penalty == 0.0
    print("Test successful.")

    print("Testing that penalty score of 0.5 is applied if duplicate_percentage > 0.15 for the Vector Search engine.")
    duplicate_penalty = _calculate_duplicate_percentage(
        uid=6,
    ...
    print("Testing that penalty score of 0.0 is applied if duplicate_percentage <= 0.15 for the Vector Search engine.")
    duplicate_penalty = _calculate_duplicate_percentage(
        uid=6,
